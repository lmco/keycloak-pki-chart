...
`keycloak.pki.enabled` | if `true`, enabled PKI authentication
`keycloak.pki.configMap` | name of the config map containing your keystore/truststore for PKI authentication.
`keycloak.pki.keystore.alias` | alias of the certificate within your keystore.
`keycloak.pki.keystore.password` | password of your PKI keystore.
`keycloak.pki.keystore.config_key` | key in your config map that contains your truststore.
`keycloak.pki.truststore.alias` | alias of the certificate within your truststore.
`keycloak.pki.truststore.password` | password of your PKI truststore.
`keycloak.pki.truststore.config_key` | key in your config map that contains your truststore.
...
For PKI to work, you first must have valid truststore and keystore Java keystores (JKS). If you have certificates in x509 PEM format, do the following to convert them:

**Keystore:**
```bash
openssl pkcs12 -export -in <cert-file>.pem -inkey <key-file>.pem -name keycloak -out keystore.p12
keytool -importkeystore -deststorepass password -destkeystore keystore.jks -srckeystore keystore.p12 -srcstoretype PKCS12
```

**Truststore:**
```bash
keytool -import -file <ca-cert-file>.pem -alias keycloak-ca -keystore truststore.jks
```

Now that you have your `keystore.jks` and `truststore.jks` files generated, run the following to create a config map of them.
```
kubectl create configmap --from-file=keystore.jks --from-file=truststore.jks keycloak-stores
```

***NOTE:*** you must ensure whichever ingress controller you are using is utilizing TLS pass-through so Keycloak can successfully authenticate a user's certificate.
